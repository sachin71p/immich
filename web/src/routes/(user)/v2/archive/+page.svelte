<!-- Heirloom Web V2 — Archive (WP9 slice 3).
  Buckets `{ visibility: Archive }` (classic archive page pattern) through the
  composed V2SquareTimeline. Viewer stays URL-backed through the `view` param. -->
<script lang="ts">
  ...
  const options = $derived(buildV2VisibilityOptions(AssetVisibility.Archive));
...
<section aria-label="Archive" data-testid="v2-archive">
  <V2SquareTimeline
    {timelineManager}
    {options}
    ...
    emptyTitle="No archived assets"
    emptyBody="Archived assets stay in your library but out of the timeline."
  />
</section>

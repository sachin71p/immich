<!-- Heirloom Web V2 — Recently Saved (WP9 slice 1).
  Live square timeline over the classic recently-added composition verbatim
  (Timeline visibility + stacked + partners, upload-order `CreatedAt`) per
  lead decision D2: native Recently Saved and Imports share this recency arm.
  Group/zoom come from the shared V2 URL state. Tiles deep-link to the V2
  asset URL; close restores the collection state. -->
<script lang="ts">
  ...
  import { buildV2RecencyOptions, v2CollectionPath } from '$lib/heirloom/route-options';
  ...
  const assetHref = (id: string) => `/v2/recently-saved/${id}`;
...
<section aria-label="Recently Saved" data-testid="v2-recently-saved">
  <V2SquareTimeline
    {timelineManager}
    {options}
    ...
    emptyBody="Recently saved assets will appear here."
  />
</section>

<!-- Heirloom Web V2 — Videos (WP9 slice 3).
  Search-backed VIDEO listing: `getTimeBuckets` carries no `type` param (WP2
    ...
    try {
      const { assets: paged } = await searchAssets({
        metadataSearchDto: {
          ...
          page: nextPage,
          withExif: true,
          ...buildV2MediaSearchFilter('videos', scope),
        },
      });
...
<section aria-label="Videos" data-testid="v2-media-videos">
  ...
  <p data-testid="v2-media-count">{results.length} videos</p>
  <V2SquareTimeline
    {assets}
    group="all"
    zoom={viewState.zoom}
    ...
    emptyBody="Videos you add will appear here."
  />
